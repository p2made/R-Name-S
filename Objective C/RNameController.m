...
{
	int iExtensions = (int)[myAExtensionsList count];
...
- (void) loadPrefs
{
	...
	[self setRenamingKindViewToBox: (int)[[myPubtnRenamingMethods selectedItem] tag]];
	...
	[myTxtfldRemvCharRangeFromNumber setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_RANGE_FROM_NUMBER]];
	[myStprRemvCharRangeFromNumber setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_RANGE_FROM_NUMBER]];
	[myTxtfldRemvCharRangeNumOfChars setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_RANGE_NUM_OF_CHARS]];
	[myStprRemvCharRangeNumOfChars setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_RANGE_NUM_OF_CHARS]];
	...
	[myTxtfldRemvCharBegOrEndNumberOfChars setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_BEG_OR_END_NUM_OF_CHARS]];
	[myStprRemvCharBegOrEndNumberOfChars setIntValue: (int)[udDefaults integerForKey: UDKEY_REMV_CHAR_BEG_OR_END_NUM_OF_CHARS]];

	[myBtnSeqNumPreserveExtension setState: [udDefaults integerForKey: UDKEY_SEQ_NUM_PRESERVE_EXTENSION]];
	if ([udDefaults integerForKey: UDKEY_SEQ_NUM_PRESERVE_EXTENSION] == NSControlStateValueOn) {
	...
	[myStprSeqNumFirstNumber setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_FIRST_NUM]];
	[myTxtfldSeqNumFirstNumber setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_FIRST_NUM]];
	[myStprSeqNumLeastFigures setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_LEAST_FIGS]];
	[myTxtfldSeqNumLeastFigures setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_LEAST_FIGS]];
	[myStprSeqNumStepValue setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_STEP_VAL]];
	[myTxtfldSeqNumStepValue setIntValue: (int)[udDefaults integerForKey: UDKEY_SEQ_NUM_STEP_VAL]];
}
...
{
	if ([myBtnclPrefClearListAutomatically state] == NSControlStateValueOn) {
...
- (void) awakeFromNib
{
	[myWindowMain registerForDraggedTypes: [NSArray arrayWithObject: NSFilenamesPboardType]];
	...
	forTableColumn: (NSTableColumn *)tableColumn row: (int)rowIndex
{
	if ([[tableColumn identifier] isEqualToString: TABLE_COLUMN_ID_ONOFF]) {
		[myFrRenamer setActualRenaming: ([object intValue] == NSControlStateValueOn)
							   AtIndex: rowIndex];
	}
}
...
{
	return (int)[myAExtensionsList count];
}
...
{
	RenamingKinds rkKind = (int)[[sender selectedItem] tag];
...
- (IBAction)showNewNames:(id)sender
{
	...
	RenamingKinds rkKind = (int)[[myPubtnRenamingMethods selectedItem] tag];
	switch (rkKind) {
		case FIND_AND_REPLACE:
		{
			BOOL bRegExp = ([myBtnFindRepRegularExpression state] == NSControlStateValueOn) ? YES : NO;
			BOOL bCaseInsensitive = ([ myBtnFindRepCaseInsensitive state] == NSControlStateValueOn) ? YES : NO;
			BOOL bReplacingAllFound = ([myBtnFindRepReplaceAllFound state] == NSControlStateValueOn) ? YES : NO;
			BOOL bPreservingExtension = ([myBtnFindRepPreserveExtension state] == NSControlStateValueOn) ? YES : NO;
		...
		case NUMBER_SEQUENTIALLY:
		{
			...
			BOOL bSameFigures = ([myBtnSeqNumSameFigures state] == NSControlStateValueOn) ? YES : NO;
			NSString *strExtension;
		...
		case CHANGE_CHARACTERS_TO_ALL_UPPERCASES:
		{
			BOOL bPreservingExtension = ([myBtnChangeCasesPreserveExtension state] == NSControlStateValueOn) ? YES : NO;
		...
		case REMOVE_CHARACTERS_FROM_BEGINNING:
		{
			...
			BOOL bPreservingExtension = ([myBtnRemvCharBegOrEndPreserveExtension state] == NSControlStateValueOn) ? YES : NO;
		...
		case REMOVE_CHARACTERS_AT_RANGE:
		{
			BOOL bPreservingExtension = ([myBtnRemvCharRangePreserveExtension state] == NSControlStateValueOn) ? YES : NO;
...
{
	[myFrRenamer sortPathPairsOrderBy: (int)[[sender selectedItem] tag]];
	[myTblviewFilesList reloadData];
}
